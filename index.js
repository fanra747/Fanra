...
async function startFanraBot() {
	...
	const { state, saveCreds } = await useMultiFileAuthState('fanradev');
	...
	await Solving(fanra, store)
	
	naze.ev.on('creds.update', saveCreds)
	
	fanra.ev.on('connection.update', async (update) => {
		...
		if (connection == 'open') {
			...
			if (global.db?.set[botNumber] && !global.db?.set[botNumber]?.join) {
				if (my.ch.length > 0 && my.ch.includes('@newsletter')) {
					if (my.ch) await fanra.newsletterMsg(my.ch, { type: 'follow' }).catch(e => {})
	...
	fanra.ev.on('contacts.update', (update) => {
		for (let contact of update) {
			let id = fanra.decodeJid(contact.id)
			if (store && store.contacts) store.contacts[id] = { id, name: contact.notify }
		}
	});
	
	fanra.ev.on('call', async (call) => {
		let botNumber = await fanra.decodeJid(naze.user.id);
		if (global.db?.set[botNumber]?.anticall) {
			for (let id of call) {
				if (id.status === 'offer') {
					let msg = await fanra.sendMessage(id.from, { text: `Saat Ini, Kami Tidak Dapat Menerima Panggilan ${id.isVideo ? 'Video' : 'Suara'}.\nJika @${id.from.split('@')[0]} Memerlukan Bantuan, Silakan Hubungi Owner :)`, mentions: [id.from]});
					await fanra.sendContact(id.from, global.owner, msg);
					await fanra.rejectCall(id.id, id.from)
				}
			}
		}
	});
	...
	naze.ev.on('messages.upsert', async (message) => {
		await MessagesUpsert(fanra, message, store, groupCache);
	});
	
	fanra.ev.on('groups.update', async (update) => {
		await GroupCacheUpdate(fanra, update, store, groupCache);
	});
	
	fanra.ev.on('group-participants.update', async (update) => {
		await GroupParticipantsUpdate(fanra, update, store, groupCache);
	});
	
	setInterval(async () => {
		await fanra.sendPresenceUpdate('available', naze.decodeJid(naze.user.id)).catch(e => {})
	}, 10 * 60 * 1000);
